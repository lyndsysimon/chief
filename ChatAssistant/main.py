...
def example_flow() -> None:
    ...
    try:
        ...
        LOGGER.info(
            "ElevenLabs TTS not fully configured for example flow, falling back to tone: %s",
            exc,
        )
    except Exception:  # pragma: no cover - defensive fallback for demo flow
        LOGGER.exception(
            "Unexpected ElevenLabs error during example flow configuration; falling back to tone"
        )
